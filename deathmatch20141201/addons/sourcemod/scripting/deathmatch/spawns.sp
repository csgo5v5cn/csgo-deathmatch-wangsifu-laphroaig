...
{     
    ...
    {
        LogMessage("IsTriggered function offset for spawns not found! Resuming without spawns triggered check feature");
        return;
    }
...
stock spawns_Load(const String:mapname[]="")
{
    ...
    while (!IsEndOfFile(fileHandle) && ReadFileLine(fileHandle, sLine, sizeof(sLine)))
    {
        ...
        else
        {
            ...
            if(iElementsFound >= 7)
            {
                if(StrStartWith(sLineParts[6], "T"))
                {
                    g_iSpawns_Team[g_iSpawns_PointCount] = Spawns_TeamT;
                }
                else if(StrStartWith(sLineParts[6], "CT"))
                {
